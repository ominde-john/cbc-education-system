...
const RoleSchoolFields = ({ form, setForm, availableRoles, schools, currentUserRole }: { form: AddUserForm; setForm: (f: AddUserForm) => void; availableRoles: string[]; schools: SchoolOption[]; currentUserRole: string; isEdit?: boolean }) => (
  <>
  ...
  return (
    <div className="w-full space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-4 justify-between">
        <div className="flex flex-1 gap-3 flex-wrap min-w-0">
          ...
          <Select value={roleFilter} onValueChange={setRoleFilter}>
            <SelectTrigger className="w-[160px] flex-shrink-0"><SelectValue placeholder="Role" /></SelectTrigger>
            <SelectContent>
          ...
          <Select value={statusFilter} onValueChange={setStatusFilter}>
            <SelectTrigger className="w-[160px] flex-shrink-0"><SelectValue placeholder="Status" /></SelectTrigger>
            <SelectContent>
        ...
        <div className="flex gap-2 flex-shrink-0">
          <Button variant="outline" size="sm" onClick={() => refetch()}><Download className="h-4 w-4 mr-2" />Refresh</Button>
          <Button size="sm" onClick={() => { setAddForm({ ...emptyForm }); setShowAdvanced(false); setAddOpen(true); }}>
                      ...
                      <TableCell className="flex-shrink-0">
                        <div className="flex items-center gap-3 min-w-0">
                          ...
                          <div className="flex items-center gap-1.5 min-w-0">
                            ...
                            {locked && (<Tooltip><TooltipTrigger><Lock className="h-3.5 w-3.5 text-destructive flex-shrink-0" /></TooltipTrigger><TooltipContent>Account Locked</TooltipContent></Tooltip>)}
                            {user.email_verified && (<Tooltip><TooltipTrigger><CheckCircle2 className="h-3.5 w-3.5 text-chart-4 flex-shrink-0" /></TooltipTrigger><TooltipContent>Email Verified</TooltipContent></Tooltip>)}
                            {user.two_factor_enabled && (<Tooltip><TooltipTrigger><Smartphone className="h-3.5 w-3.5 text-primary flex-shrink-0" /></TooltipTrigger><TooltipContent>2FA Enabled</TooltipContent></Tooltip>)}
                            {user.login_attempts > 3 && (
                              <Tooltip><TooltipTrigger>
                                <span className="inline-flex items-center justify-center h-4 w-4 rounded-full bg-destructive text-destructive-foreground text-[10px] font-bold flex-shrink-0">{user.login_attempts}</span>
                              </TooltipTrigger><TooltipContent>Failed login attempts: {user.login_attempts}</TooltipContent></Tooltip>
                            )}
                          </div>
                        </div>
                      </TableCell>
                      <TableCell className="text-muted-foreground truncate flex-shrink-0">{user.email}</TableCell>
                      <TableCell className="flex-shrink-0"><Badge variant="outline" className={roleBadgeClasses[user.role]}>{roleLabels[user.role]}</Badge></TableCell>
                      <TableCell className="flex-shrink-0">{user.school_name ? <span className="text-sm">{user.school_name}</span> : <span className="text-xs text-muted-foreground italic">Platform Level</span>}</TableCell>
                      <TableCell className="flex-shrink-0"><Badge variant={status.variant}>{status.label}</Badge></TableCell>
                      <TableCell className="text-sm text-muted-foreground flex-shrink-0">
                        {user.last_login ? format(new Date(user.last_login), 'MMM d, yyyy HH:mm') : <span className="italic text-xs">Never Logged In</span>}
                      </TableCell>
                      <TableCell className="flex-shrink-0"><span className="text-sm font-mono">{user.active_sessions}/{user.max_sessions}</span></TableCell>
      ...
      <Dialog open={addOpen} onOpenChange={setAddOpen}>
        <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto rounded-lg">
          ...
          <div className="space-y-4 py-2">
            ...
            <Separator />
            <RoleSchoolFields form={addForm} setForm={setAddForm} availableRoles={availableRoles} schools={schools} currentUserRole={currentUserRole} />
      ...
      <Dialog open={!!viewUser} onOpenChange={() => setViewUser(null)}>
        <DialogContent className="sm:max-w-md max-h-[90vh] overflow-y-auto rounded-lg">
          <DialogHeader>
            ...
            <div className="space-y-1">
              <div className="flex items-center gap-3 pb-3">
                ...
                <div className="min-w-0">
                  <p className="font-semibold text-lg truncate">{viewUser.first_name} {viewUser.last_name}</p>
      ...
      <Dialog open={!!editUser} onOpenChange={() => setEditUser(null)}>
        <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto rounded-lg">
          ...
          <div className="space-y-4 py-2">
            ...
            <Separator />
            <RoleSchoolFields form={editForm} setForm={setEditForm} availableRoles={availableRoles} schools={schools} currentUserRole={currentUserRole} isEdit />
            <Separator />
            <div className="space-y-2">
